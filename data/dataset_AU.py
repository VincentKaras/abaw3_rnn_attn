...
class AU_Dataset(BaseDataset):
    """
    Constructs a dataset for the AU Challenge.
    ...
    def __init__(self, dataset_file: Path, seq_len, fps, audio_window_size, audio_sr, train_mode="Train", channel_order="RGB",
                 transforms=None):
        super(AU_Dataset, self).__init__(train_mode=train_mode, transforms=transforms, seq_len=seq_len, fps=fps)
        # overwrite the name
        self.audio_sr = audio_sr
        self.audio_window_size = audio_window_size
        self._name = "AU_dataset"
        
        self.channel_order = channel_order

        # parse the sequences
        self._parse_dataset_paths(dataset_file=dataset_file, task="AU")
    ...
    def __getitem__(self, item):
        ...
        for i, row in df.iterrows():
            ...
            label = row[PATHS.tasks["AU"]].values.astype(np.float32)    # get the valence and arousal entries for this timestamp
